...
pub trait BezoutDomain : Ring {
    ...
    fn l0_matrix(&self, other: &Self) -> [[Self;2];2] {
        ...
        let alpha = self.divide_by_divisor(&beta).expect("Dividing by the GCD which is a divisor");
        let gamma = other.divide_by_divisor(&beta).expect("Dividing by the GCD which is a divisor");
